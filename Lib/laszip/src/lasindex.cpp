...
BOOL LASindex::append(const char* file_name) const
{
	...
	if ((number_of_special_evlrs == -1) && (offset_to_special_evlrs == -1))
	{
		...
		for (U32 u = 0; u < number_of_variable_length_records; u++)
		{
			...
			CHAR user_id[16];
			try { bytestreamin->getBytes((U8*)user_id, 16); }
			catch (...)
			{
			...
			U16 record_id;
			try { bytestreamin->get16bitsLE((U8*)&record_id); }
			catch (...)
			{
			...
			try { bytestreamin->get16bitsLE((U8*)&record_length_after_header); }
			catch (...)
			{
...
BOOL LASindex::read(ByteStreamIn* stream)
{
	...
	char signature[4];
	try { stream->getBytes((U8*)signature, 4); }
	catch (...)
	{
	...
	U32 version;
	try { stream->get32bitsLE((U8*)&version); }
	catch (...)
	{
...
bool LASindex::merge_intervals()
{
	if (spatial->get_intersected_cells())
	{
		...
		if (used_cells)
		{
			bool r = (interval->merge() == TRUE ? true : false);
			full = interval->full;
