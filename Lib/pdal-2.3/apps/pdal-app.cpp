...
void App::addArgs(ProgramArgs& args)
{
    ...
    args.add("debug", "Sets the output level to 3 (option deprecated)", m_debug);
    args.add("verbose,v", "Sets the output level (0-8)", m_logLevel, LogLevel::None);
    ...
    args.add("options", "Show options for specified driver (or 'all')", m_showOptions);
    args.add("log", "Log filename (accepts stderr, stdout, stdlog, devnull as special cases)", m_log, "stderr");
    args.add("logtiming", "Turn on timing for log messages", m_logtiming);
    Arg& json = args.add("showjson", "List options or drivers as JSON output", m_showJSON);
    json.setHidden();
}
...
//        cmdArgs.push_back(argv[i]);
    ...
    // info command
    //cmdArgs.push_back("info");
    //cmdArgs.push_back("D:/Geodata/lidar/100-points.las");

    // dtm pipeline
    cmdArgs.push_back("pipeline");
    cmdArgs.push_back("C:/DevLib/pdal-2.3/apps/pipeline-dtm.txt");
