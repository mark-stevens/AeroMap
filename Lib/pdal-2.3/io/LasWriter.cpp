...
#include <pdal/compression/LazPerfVlrCompression.h>
...
namespace pdal
{
	...
	void LasWriter::prepared(PointTableRef table)
	{
		...
		for (auto& dim : m_extraDims)
		{
			...
			m_extraByteLen += (uint16_t)Dimension::size(dim.m_dimType.m_type);
	...
	void LasWriter::fillHeader()
	{
		...
		m_lasHeader.setVlrCount((uint32_t)m_vlrs.size());
		m_lasHeader.setEVlrCount((uint32_t)m_eVlrs.size());
	...
	bool LasWriter::fillPointBuf(PointRef& point, LeInserter& ostream)
	{
		...
		if (numberOfReturns > maxReturnCount)
		{
			if (m_discardHighReturnNumbers)
			{
				...
				if (returnNumber > maxReturnCount)
					return false;
				numberOfReturns = (uint8_t)maxReturnCount;
			}
		}
