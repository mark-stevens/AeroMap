...
namespace pdal
{
	...
	void BpfWriter::initialize()
	{
		...
		for (auto file : m_bundledFilesSpec)
		{
			...
			BpfUlemFile ulemFile((uint32_t)size, FileUtils::getFilename(file), file);
			if (ulemFile.m_filename.length() > 32)
				throwError("Bundled file '" + file + "' name exceeds maximum length of 32.");
		...
		const SpatialReference& srs)
	{
		...
		m_header.m_numDim = (int32_t)m_dims.size();
		m_header.m_numPts = 0;
		m_header.setLog(log());
