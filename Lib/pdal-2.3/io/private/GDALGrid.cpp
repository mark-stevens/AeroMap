...
namespace pdal
{
		...
		, m_outputTypes(outputTypes)
	{
		...
		RasterLimits limits(xOrigin, yOrigin, (int)width, (int)height, edgeLength);
	...
	*/
	...
	void GDALGrid::update(size_t i, size_t j, double val, double dist)
	{
		...
		double& count = m_count->at((int)i, (int)j);
		count++;

		if (m_min)
		{
			double& min = m_min->at((int)i, (int)j);
			min = (std::min)(val, min);
		}

		if (m_max)
		{
			double& max = m_max->at((int)i, (int)j);
			max = (std::max)(val, max);
		}

		if (m_mean)
		{
			double& mean = m_mean->at((int)i, (int)j);
			double delta = val - mean;

			mean += delta / count;
			if (m_stdDev)
			{
				double& stdDev = m_stdDev->at((int)i, (int)j);
		...
		if (m_idw)
		{
			double& idw = m_idw->at((int)i, (int)j);
			double& idwDist = m_idwDist->at((int)i, (int)j);
