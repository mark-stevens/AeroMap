...
namespace pdal
{
	...
	struct XForm
	{
		struct XFormComponent
		{
			...
			friend std::ostream& operator<<(std::ostream& in, const XFormComponent& xfc);
		};
		...
		XForm(double scale, double offset) 
			: m_scale(scale)
			, m_offset(offset)
		{
		}
	...
	inline std::ostream& operator<<(std::ostream& out, const XForm::XFormComponent& xfc)
	{
		if (xfc.m_auto)
			out << "auto";
		else
