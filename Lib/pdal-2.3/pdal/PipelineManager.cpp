...
namespace pdal
{
	PipelineManager::PipelineManager(point_count_t streamLimit)
		...
		, m_tablePtr(new ColumnPointTable())
		, m_table(*m_tablePtr)
	...
	Stage& PipelineManager::makeReader(const std::string& inputFile, std::string driver)
	{
	...
	Stage& PipelineManager::makeFilter(const std::string& driver, Stage& parent, Options options)
	{
	...
	Stage& PipelineManager::makeWriter(const std::string& outputFile, std::string driver)
	{
