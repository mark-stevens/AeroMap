...
namespace pdal
{
	...
	{
		args.add("conversion", "time conversion type", m_conversion).setPositional();
		args.add("start_date", "GMT start date of data in 'YYYY-MM-DD' format", m_strDate, "");
		args.add("wrap", "reset output week seconds to zero on Sundays", m_wrap, false);
		args.add("wrapped", "input weeks seconds reset to zero on Sundays", m_wrapped, false);
	}
