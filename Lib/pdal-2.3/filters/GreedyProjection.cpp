...
namespace pdal
{
	...
	{
		args.add("multiplier", "Nearest neighbor distance multiplier", mu_).setPositional();
		args.add("radius", "Search radius for neighbors", search_radius_).setPositional();
		args.add("num_neighbors", "Number of nearest neighbors to consider", nnn_, 100);
		args.add("min_angle", "Minimum angle for created triangles", minimum_angle_, M_PI / 18);  // 10 degrees default
		args.add("max_angle", "Maximum angle for created triangles", maximum_angle_, 2 * M_PI / 3);  // 120 degrees default
		args.add("eps_angle", "Max normal difference angle for triangulation consideration", eps_angle_, M_PI / 4);
	}

	void GreedyProjection::addDimensions(PointLayoutPtr layout)
	{
		layout->registerDims({ Dimension::Id::NormalX, Dimension::Id::NormalY, Dimension::Id::NormalZ });
	}
