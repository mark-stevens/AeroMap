...
namespace pdal
{
	...
	void SplitterFilter::addArgs(ProgramArgs& args)
	{
		...
		args.add("origin_x", "X origin for a cell", m_xOrigin, std::numeric_limits<double>::quiet_NaN());
		args.add("origin_y", "Y origin for a cell", m_yOrigin, std::numeric_limits<double>::quiet_NaN());
		args.add("buffer", "Size of buffer (overlap) to include around each tile.", m_buffer, 0.0);
	}
