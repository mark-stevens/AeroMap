...
int StageDataset::InitGeoref()
{
	...
	for (Project::ImageType image : GetProject().GetImageList())
	{
		...
		if (utm_zone == 0)
		{
			utm_zone = GIS::GetUTMZone(image.exif.GeoLocation.Longitude);
			hemi = image.exif.GeoLocation.Latitude >= 0.0 ? 'N' : 'S';
		}

		double x, y;
		GIS::LatLonToXY_UTM(image.exif.GeoLocation.Latitude, image.exif.GeoLocation.Longitude, x, y, GIS::Ellipsoid::WGS_84);
