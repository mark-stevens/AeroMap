...
XString Cropper::create_bounds_geojson(XString pointcloud_path, double buffer_distance, int decimation_step)
{
	...
	json pc_geojson_boundary_feature = data["boundary"]["boundary_json"];
	...
	if (pFile)
	{
		...
		fprintf(pFile, "	\"geometry\": pc_geojson_boundary_feature\n");
		fprintf(pFile, "	}]\n");
		fprintf(pFile, "}))\n");
		fclose(pFile);
	}
	...
	GDALDriver* pDriver = static_cast<GDALDriver*>(GDALGetDriverByName("GeoJSON"));
	assert(pDriver != nullptr);

	GDALDataset* pDS = (GDALDataset*)GDALOpen(tmp_bounds_geojson_path.c_str(), GA_ReadOnly);
	assert(pDS != nullptr);
	...
	if (QFile::exists(bounds_geojson_path.c_str()))
		QFile::remove(bounds_geojson_path.c_str());
...
void Cropper::merge_bounds(XString input_bound_files, XString output_bounds, double buffer_distance)
{
