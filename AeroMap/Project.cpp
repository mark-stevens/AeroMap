...
void Project::LoadData()
{
	...
	for (int i = 0; i < (int)m_LidarList.size(); ++i)
	{
		if (m_LidarList[i].exists)
			m_LidarList[i].pFile = new LasFile(m_LidarList[i].src.c_str());
	}
	emit projectChanged_sig();
}
...
int Project::Save(const char* fileName)
{
	...
	if (pFile != nullptr)
	{
		...
		// lidar section
		fprintf(pFile, "lidar\n");
		for (int i = 0; i < m_LidarList.size(); ++i)
		{
			fprintf(pFile, "\tsrc = %s\n", m_LidarList[i].src.c_str());
		}
...
{
	GetApp()->LogWrite("Load image list...");
	...
	for (int i = 0; i < list.size(); ++i)
	{
		...
		if (fileInfo.isFile())
		{
			...
			{
				Logger::Write(__FUNCTION__, "Unable to read image file: '%s'", file_path.c_str());
				delete[] buf;
				return -2;
			}
			fclose(fp);
...
void Project::RemoveLidarFile(int index)
{
	...
	if (index >= (int)m_LidarList.size())
		return;

	m_LidarList.erase(m_LidarList.cbegin() + index);
...
{
	if (index >= 0 && index < GetLidarFileCount())
		return m_LidarList[index].pFile;
...
{
	if (index >= 0 && index < GetLidarFileCount())
		return m_LidarList[index].exists;
...
void Project::InitArg()
{
	...
	//				Number of steps used to fill areas with gaps. Set to 0 to disable gap filling. Starting with a radius equal to the output resolution, N different DEMs are generated with progressively bigger radius using the
	//				inverse distance weighted (IDW) algorithm and merged together. Remaining gaps are then merged using nearest neighbor interpolation. Default: 3
	...
	//				Decimate the points before generating the DEM. 1 is no decimation (full quality). 100 decimates ~99% of the points. Useful for speeding up generation of DEM results 
	//				in very large datasets. Default: 1
	arg.dem_euclidean_map = false;
	// --dem-euclidean-map   
	//				Computes an euclidean raster map for each DEM. The map reports the distance from each cell to the nearest NODATA value (before any hole filling takes place). This can be useful to isolate the areas that have
	...
	// --fast-orthophoto     
	//				Skips dense reconstruction and 3D model generation. It generates an orthophoto directly from the sparse reconstruction. If you just need an orthophoto and do not need a full 
	//				3D model, turn on this option.
	//				Default: False
	...
	// --ignore-gsd 
	//				Ignore Ground Sampling Distance (GSD).A memory and processor hungry change relative to the default behavior if set to true. Ordinarily, GSD estimates are used to cap the 
	//				maximum resolution of image outputs and resizes images when necessary, resulting in faster processing and lower memory usage. Since GSD is an estimate, sometimes ignoring
	//				it can result in slightly better image output quality. Never set --ignore-gsd to true unless you are positive you need it, and even then: do not use it. 
	//				Default: False
	...
	//				Filters the point cloud by keeping only a single point around a radius N (in meters). This can be useful to limit the output resolution of the point cloud 
	//				and remove duplicate points. Set to 0 to disable
	//				sampling. Default: 0
...
//core count?
	...
	//				The maximum number of processes to use in various processes. Peak memory requirement is ~1GB per thread and 2 megapixel image resolution. Default: 16

	// --min-num-features <integer>
	//				Minimum number of features to extract per image. More features can be useful for finding more matches between images, potentially allowing the reconstruction of areas with little overlap or insufficient
	//				features. More features also slow down processing. Default: 10000
	// --feature-type <string>
	//				Choose the algorithm for extracting keypoints and computing descriptors. Can be one of: akaze, hahog, orb, sift. Default: sift
	...
	// --matcher-type <string>
	//				Matcher algorithm, Fast Library for Approximate Nearest Neighbors or Bag of Words. FLANN is slower, but more stable. BOW is faster, but can sometimes miss valid matches. BRUTEFORCE is very slow but robust.Can be
	//				one of: bow, bruteforce, flann. Default: flann
	...
	//				Perform image matching with the nearest N images based on image filename order. Can speed up processing of sequential images, such as those extracted from video. It is applied only on non-georeferenced datasets.
	//				Set to 0 to disable. Default: 0
	// --use-fixed-camera-params
	//				Turn off camera parameter optimization during bundle adjustment. This can be sometimes useful for improving results that exhibit doming/bowling or when images are taken with a rolling shutter camera. Default:
	//				False
	// --cameras <json>      Use the camera parameters computed from another dataset instead of calculating them. Can be specified either as path to a cameras.json file or as a JSON string representing the contents of a cameras.json file.
	//				Default:
	// --camera-lens <string>
	//				Set a camera projection type. Manually setting a value can help improve geometric undistortion. By default the application tries to determine a lens type from the images metadata. Can be one of: auto,
	//				perspective, brown, fisheye, fisheye_opencv, spherical, equirectangular, dual. Default: auto
	// --radiometric-calibration <string>
	//				Set the radiometric calibration to perform on images. When processing multispectral and thermal images you should set this option to obtain reflectance/temperature values (otherwise you will get digital number
	//				values). [camera] applies black level, vignetting, row gradient gain/exposure compensation (if appropriate EXIF tags are found) and computes absolute temperature values. [camera+sun] is experimental, applies all
	//				the corrections of [camera], plus compensates for spectral radiance registered via a downwelling light sensor (DLS) taking in consideration the angle of the sun. Can be one of: none, camera, camera+sun. Default:
	//				none
	...
	//				Choose the structure from motion algorithm. For aerial datasets, if camera GPS positions and angles are available, triangulation can generate better results. For planar scenes captured at fixed altitude with
	//				nadir-only images, planar can be much faster. Can be one of: incremental, triangulation, planar. Default: incremental
	// --sfm-no-partial      Do not attempt to merge partial reconstructions. This can happen when images do not have sufficient overlap or are isolated. Default: False
	// --sky-removal         Automatically compute image masks using AI to remove the sky. Experimental. Default: False
	// --bg-removal          Automatically compute image masks using AI to remove the background. Experimental. Default: False
	...
	// --boundary <json>     GeoJSON polygon limiting the area of the reconstruction. Can be specified either as path to a GeoJSON file or as a JSON string representing the contents of a GeoJSON file. Default:
	// --auto-boundary       Automatically set a boundary using camera shot locations to limit the area of the reconstruction. This can help remove far away background artifacts (sky, background landscapes, etc.). See also --boundary.
	//				Default: False
	...
	// --pc-quality <string>
	//				Set point cloud quality. Higher quality generates better, denser point clouds, but requires more memory and takes longer. Each step up in quality increases processing time roughly by a factor of 4x.Can be one
	//				of: ultra, high, medium, low, lowest. Default: medium
	...
	// --pc-skip-geometric   Geometric estimates improve the accuracy of the point cloud by computing geometrically consistent depthmaps but may not be usable in larger datasets. This flag disables geometric estimates. Default: False
	...
	// --gcp <path string>   Path to the file containing the ground control points used for georeferencing. The file needs to use the following format: EPSG:<code> or <+proj definition> geo_x geo_y geo_z im_x im_y image_name [gcp_name]
	//				[extra1] [extra2] Default: None
	// --geo <path string>   Path to the image geolocation file containing the camera center coordinates used for georeferencing. If you don't have values for yaw/pitch/roll you can set them to 0. The file needs to use the following format:
	//				EPSG:<code> or <+proj definition> image_name geo_x geo_y geo_z [yaw (degrees)] [pitch (degrees)] [roll (degrees)] [horz accuracy (meters)] [vert accuracy (meters)] Default: None
	// --align <path string>
	...
	// --orthophoto-cutline  Generates a polygon around the cropping area that cuts the orthophoto around the edges of features. This polygon can be useful for stitching seamless mosaics with multiple overlapping orthophotos. Default: False
	...
	// --rolling-shutter     Turn on rolling shutter correction. If the camera has a rolling shutter and the images were taken in motion, you can turn on this option to improve the accuracy of the results. See also --rolling-shutter-
	//				readout. Default: False
	// --rolling-shutter-readout <positive integer>
	//				Override the rolling shutter readout time for your camera sensor (in milliseconds), instead of using the rolling shutter readout database. Note that not all cameras are present in the database. Set to 0 to use
	...
	//				Radius of the overlap between submodels. After grouping images into clusters, images that are closer than this radius to a cluster are added to the cluster. This is done to ensure that neighboring submodels
	//				overlap. Default: 150
	// --split-image-groups <path string>
	//				Path to the image groups file that controls how images should be split into groups. The file needs to use the following format: image_name group_name Default: None
	// --sm-no-align         Skip alignment of submodels in split-merge. Useful if GPS is good enough on very large datasets. Default: False
	// --sm-cluster <string>
	//				URL to a ClusterODM instance for distributing a split-merge workflow on multiple nodes in parallel. Default: None
	// --merge <string>      Choose what to merge in the merge step in a split dataset. By default all available outputs are merged. Options: all, pointcloud, orthophoto, dem. Default: all
	// --force-gps           Use images' GPS exif data for reconstruction, even if there are GCPs present.This flag is useful if you have high precision GPS measurements. If there are no GCPs, this flag does nothing. Default: False
	// --gps-accuracy <positive float>
	//				Set a value in meters for the GPS Dilution of Precision (DOP) information for all images. If your images are tagged with high precision GPS information (RTK), this value will be automatically set accordingly.
	...
	// --optimize-disk-space
	//				Delete heavy intermediate files to optimize disk space usage. This affects the ability to restart the pipeline from an intermediate stage, but allows datasets to be processed on machines that don't have
	...
	//				End processing at this stage. Can be one of: dataset, split, merge, opensfm, openmvs, odm_filterpoints, odm_meshing, mvs_texturing, odm_georeferencing, odm_dem, odm_orthophoto, odm_report, odm_postprocess.
	//				Default: odm_postprocess
	// --rerun <string>, -r <string>
	//				Rerun this stage only and stop. Can be one of: dataset, split, merge, opensfm, openmvs, odm_filterpoints, odm_meshing, mvs_texturing, odm_georeferencing, odm_dem, odm_orthophoto, odm_report, odm_postprocess.
	//				Default: None
	// --rerun-all           Permanently delete all previous results and rerun the processing pipeline.
	// --rerun-from <string>
	//				Rerun processing from this stage. Can be one of: dataset, split, merge, opensfm, openmvs, odm_filterpoints, odm_meshing, mvs_texturing, odm_georeferencing, odm_dem, odm_orthophoto, odm_report, odm_postprocess.
	//				Default: None
}
