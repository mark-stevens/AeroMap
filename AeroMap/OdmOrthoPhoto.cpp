...
int OdmOrthoPhoto::CreateOrthoPhoto()
{
	if (m_InputFiles.size() == 0)
	{
		Logger::Write(__FUNCTION__, "Failed to create ortho photo, no texture meshes given.");
		return -1;
	}
	...
	//    Bounds b = computeBoundsForModel(mesh);
	    ...
	    //Logger::Write(__FUNCTION__, "Model bounds x: %0.3f -> %0.3f", b.xMin, b.xMax);
	    //Logger::Write(__FUNCTION__, "Model bounds y: %0.3f -> %0.3f", b.yMin, b.yMax);
	...
	//        }
		...
		Logger::Write(__FUNCTION__, "Model area : %0.2f m~2", xDiff * yDiff);

	    // The resolution necessary to fit the area with the given resolution.
	    height = static_cast<int>(std::ceil(m_Resolution*yDiff));
	    width = static_cast<int>(std::ceil(m_Resolution*xDiff));

	    depth_ = cv::Mat::zeros(height, width, CV_32F) - std::numeric_limits<float>::infinity();
	    Logger::Write(__FUNCTION__, "Model resolution, width x height : %d x %d", width, height);
		...
		if (0 >= height * width)
		{
			if (0 >= height)
			{
				Logger::Write(__FUNCTION__, "Warning: ortho photo has zero area, height = %d. Forcing height = 1.", height);
				height = 1;
			}
			if (0 >= width)
			{
				Logger::Write(__FUNCTION__, "Warning: ortho photo has zero area, width = %d. Forcing width = 1.");
				width = 1;
			}
			Logger::Write(__FUNCTION__, "New ortho photo resolution, width x height: %d x %d", width, height);
		}

		// Contains the vertices of the mesh.
	//    pcl::PointCloud<VEC3>::Ptr meshCloud (new pcl::PointCloud<VEC3>);
	//    pcl::fromPCLPointCloud2 (mesh.cloud, *meshCloud);

	    // Split model and make copies of vertices and texture coordinates for all faces
	    if (splitModel)
	    {
	//        pcl::PointCloud<VEC3>::Ptr meshCloudSplit (new pcl::PointCloud<VEC3>);
	...
	//                VEC3 v1 = meshCloud->points[v1i];
	//                VEC3 v2 = meshCloud->points[v2i];
	//                VEC3 v3 = meshCloud->points[v3i];
	...
	//    Logger::Write(__FUNCTION__, "Translating and scaling mesh...\n";
	...
	//            Logger::Write(__FUNCTION__, "Texture channels: " << texture.channels() << "\n";

	//            try{
	//                if (textureDepth == CV_8U){
	//                    Logger::Write(__FUNCTION__, "Texture depth: 8bit\n";
	//                    initBands<uint8_t>(texture.channels());
	//                    if (primary) initAlphaBand<uint8_t>();
	//                }else if (textureDepth == CV_16U){
	//                    Logger::Write(__FUNCTION__, "Texture depth: 16bit\n";
	//                    initBands<uint16_t>(texture.channels());
	//                    if (primary) initAlphaBand<uint16_t>();
	//                }else if (textureDepth == CV_32F){
	//                    Logger::Write(__FUNCTION__, "Texture depth: 32bit (float)\n";
	...
	//        if(texture.empty())
	//        {
	//            Logger::Write(__FUNCTION__, "Material texture could not be read:\n";
	//            log_ << material.tex_file << '\n';
	//            Logger::Write(__FUNCTION__, "Could not be read as image, does the file exist?\n";
	...
	//        }
	//        faceOff += faces.size();
	//        Logger::Write(__FUNCTION__, "Material " << t << " rendered.\n";
	//    }
	//    Logger::Write(__FUNCTION__, "... model rendered\n";

	//    currentBandIndex += texture.channels();
	//    primary = false;
	}

	Logger::Write(__FUNCTION__, "Writing ortho photo to %s", m_OutputFile.c_str());

	if (textureDepth == CV_8U)
	{
	    saveTIFF(m_OutputFile, GDT_Byte);
	}
	else if (textureDepth == CV_16U)
	{
	    saveTIFF(m_OutputFile, GDT_UInt16);
	}
	else if (textureDepth == CV_32F)
	{
	...
	{
	    Logger::Write(__FUNCTION__, "Writing corner coordinates to %s", m_OutputCornerFile.c_str());
	    std::ofstream cornerStream(m_OutputCornerFile.c_str());
	    if (!cornerStream.is_open())
	    {
	        //throw OdmOrthoPhotoException("Failed opening output corner file " + m_OutputCornerFile + ".");
	    }
	...
	Logger::Write(__FUNCTION__, "Orthophoto generation done.");
	return 0;
}
...
//    }
//
//    Logger::Write(__FUNCTION__, "Boundary points:\n";
//    Logger::Write(__FUNCTION__, "Point 1: " << r.xMin << " " << r.yMin << "\n";
//    Logger::Write(__FUNCTION__, "Point 2: " << r.xMin << " " << r.yMax << "\n";
//    Logger::Write(__FUNCTION__, "Point 3: " << r.xMax << " " << r.yMax << "\n";
//    Logger::Write(__FUNCTION__, "Point 4: " << r.xMax << " " << r.yMin << "\n";
...
//
//    transform(0, 0) = m_Resolution;     // x Scaling.
//    transform(1, 0) = 0.0f;
//    transform(2, 0) = 0.0f;
//    transform(3, 0) = 0.0f;
//
//    transform(0, 1) = 0.0f;
//    transform(1, 1) = -m_Resolution;     // y Scaling, mirrored for easier rendering.
//    transform(2, 1) = 0.0f;
//    transform(3, 1) = 0.0f;
//
//    transform(0, 2) = 0.0f;
...
//    transform(3, 2) = 0.0f;
//
//    transform(0, 3) = -xMin * m_Resolution;    // x Translation
//    transform(1, 3) = -yMin * m_Resolution;    // y Translation
//    transform(2, 3) = 0.0f;
//    transform(3, 3) = 1.0f;
//
//    return transform;
...
//template <typename T>
//void OdmOrthoPhoto::drawTexturedTriangle(const cv::Mat &texture, const pcl::Vertices &polygon, const pcl::PointCloud<VEC3>::Ptr &meshCloud, const std::vector<Eigen::Vector2f> &uvs, size_t faceIndex)
//{
////     /*!
...
//    {
//        Logger::Write(__FUNCTION__, "Warning: Sliver polygon found at face index " << faceIndex << '\n';
//        return;
//    }
//
...
bool OdmOrthoPhoto::isSliverPolygon(VEC3 v1, VEC3 v2, VEC3 v3) const
{
	// Check if a given polygon is a sliver polygon.
	//
	// Inputs:
	//		v1, v2, v3 = polygon vertices
	//

	VEC3 dummyVec = CrossProduct(v1 - v2, v3 - v2);

    // Area smaller than, or equal to, floating-point epsilon.
	double area = static_cast<float>(std::sqrt(DotProduct(dummyVec, dummyVec)) / 2.0);
	return std::numeric_limits<float>::epsilon() >= area;
}
...
//    }
//    
//    Logger::Write(__FUNCTION__, "Number of faces in the model " << nFaces << '\n';
//    
...
//{
	...
	//}
	//
	//Logger::Write(__FUNCTION__, "Arguments given\n";
	...
	//        ss >> m_Resolution;
	//        Logger::Write(__FUNCTION__, "Resolution count was set to: " << m_Resolution << "pixels/meter\n";
	//    }
	...
	//        }
	//        Logger::Write(__FUNCTION__, "Log file path was set to: " << logFile_ << "\n";
	//    }
	//    else if(argument == "-m_InputFiles")
	//    {
	//        argIndex++;
	//        if (argIndex >= argc)
	//        {
	//            throw OdmOrthoPhotoException("Argument '" + argument + "' expects 1 more input following it, but no more inputs were provided.");
	//        }
	//        std::string m_InputFilesArg = std::string(argv[argIndex]);
	//        std::stringstream ss(m_InputFilesArg);
	//        std::string item;
	...
	//            throw OdmOrthoPhotoException("Argument '" + argument + "' expects 1 more input following it, but no more inputs were provided.");
	//        }
	//        m_BandsOrder = std::string(argv[argIndex]);
	//    }
	...
	//            throw OdmOrthoPhotoException("Argument '" + argument + "' expects 1 more input following it, but no more inputs were provided.");
	//        }
	//        m_OutputFile = std::string(argv[argIndex]);
	//        Logger::Write(__FUNCTION__, "Writing output to: " << m_OutputFile << "\n";
	//    }
	...
	//            throw OdmOrthoPhotoException("Argument '" + argument + "' expects 1 more input following it, but no more inputs were provided.");
	//        }
	//        m_OutputCornerFile = std::string(argv[argIndex]);
	//        Logger::Write(__FUNCTION__, "Writing corners to: " << m_OutputCornerFile << "\n";
	//    }
	//    else
	//    {
	//        printHelp();
	...
	//std::stringstream ss(m_BandsOrder);
	//std::string item;
	//while(std::getline(ss, item, ',')){
...
//{
	...
	//Logger::Write(__FUNCTION__, "odm_orthophoto\n\n";

	//Logger::Write(__FUNCTION__, "Purpose\n";
	//Logger::Write(__FUNCTION__, "Create an orthograpical photo from an oriented textured mesh.\n\n";

	//Logger::Write(__FUNCTION__, "Usage:\n";
	//Logger::Write(__FUNCTION__, "The program requires a path to an input OBJ mesh file and a resolution, as pixels/m. All other input parameters are optional.\n\n";

	//Logger::Write(__FUNCTION__, "The following flags are available\n";
	//Logger::Write(__FUNCTION__, "Call the program with flag \"-help\", or without parameters to print this message, or check any generated log file.\n";
	//Logger::Write(__FUNCTION__, "Call the program with flag \"-verbose\", to print log messages in the standard output stream as well as in the log file.\n\n";

	//Logger::Write(__FUNCTION__, "Parameters are specified as: \"-<argument name> <argument>\", (without <>), and the following parameters are configureable:\n";
	//Logger::Write(__FUNCTION__, "\"-m_InputFiles <path>[,<path2>,<path3>,...]\" (mandatory)\n";
	//Logger::Write(__FUNCTION__, "\"Input obj files that must contain a textured mesh.\n\n";

	//Logger::Write(__FUNCTION__, "\"-outputFile <path>\" (optional, default: ortho.jpg)\n";
	//Logger::Write(__FUNCTION__, "\"Target file in which the orthophoto is saved.\n\n";

	//Logger::Write(__FUNCTION__, "\"-outputCornerFile <path>\" (optional)\n";
	//Logger::Write(__FUNCTION__, "\"Target text file for boundary corner points, written as \"xmin ymin xmax ymax\".\n\n";

	//Logger::Write(__FUNCTION__, "\"-resolution <pixels/m>\" (mandatory)\n";
	//Logger::Write(__FUNCTION__, "\"The number of pixels used per meter.\n\n";

	//Logger::Write(__FUNCTION__, "\"-bands red,green,blue,[...]\" (optional)\n";
	//Logger::Write(__FUNCTION__, "\"Naming of bands to assign color interpolation values when creating output TIFF.\n\n";
