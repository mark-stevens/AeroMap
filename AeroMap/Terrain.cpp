...
double Terrain::DistanceToShore(int row, int col)
{
	// Distance to closest shoreline.
	//

	double distance = GetRadius() * 2.0;

	for (auto shore : mv_ShoreLines)
	{
		for (auto pt : shore.pts)
		{
			// skip points on map edge, they are not true
			// shorelines
			if (pt.row == 0 || pt.row == GetRowCount() - 1)
				continue;
			if (pt.col == 0 || pt.col == GetColCount() - 1)
				continue;

			double d = Distance((double)pt.col, (double)pt.row, (double)col, (double)row);
			if (d < distance)
				distance = d;
		}
	}
...
bool Terrain::ShoreEval(int row, int col)
{
	// Return true if point at (row,col)
	// qualifies as a shoreline point.
	//
	// Shore point defined as:
	//		land vertex with adjacent water
	//		water vertex along model boundary

	// off map
	if (IsValid(row, col) == false)
		return false;
	
	// first test for "edge" shoreline, this supercedes
	// any test of neighbors

	if (IsWater(row, col)
	&& (row == 0 || row == GetRowCount() - 1 || col == 0 || col == GetColCount() - 1))
		return true;

	if (IsLand(row, col) && IsNeighborWater(row, col))
		return true;
...
int Terrain::ExtractShoreLines()
{
	// Extract vectorized shorelines.
	//
	// Outputs:
	//		return = # of shorelines
	//

	mv_ShoreLines.clear();

	// clear existing shoreline flags
	for (int row = 0; row < GetRowCount(); ++row)
	{
		for (int col = 0; col < GetColCount(); ++col)
		{
			UInt8 flags = GetFlags(row, col);
			UInt8 shoreFlag = static_cast<UInt8>(Flags::SHORE);

			if ((flags & shoreFlag) > 0)
			{
				flags &= (~shoreFlag);
				SetFlags(row, col, flags);
			}
		}
	}
	...
	for (int row = 0; row < GetRowCount(); ++row)
	{
		for (int col = 0; col < GetColCount(); ++col)
		{
			// point evaluates to "shore"
			if (ShoreEval(row, col))
			{
				// if this point not already in a shoreline
			
				if ((GetFlags(row, col) & static_cast<UInt8>(Flags::SHORE)) == 0)
				{
					ShoreLineType shore = ExtractShoreLine(row, col);
					if (shore.pts.size() > ShoreLineType::MIN_PTS)
					{
						// add it to our list of shorelines
						mv_ShoreLines.push_back(shore);

						for (int i = 0; i < shore.pts.size(); ++i)
						{
							Logger::Write("", "Shore[%d]: (row %d, col %d)", mv_ShoreLines.size() - 1, shore.pts[i].row, shore.pts[i].col);
						}
					}
				}
			}
	...
	return static_cast<int>(mv_ShoreLines.size());
}

Terrain::ShoreLineType Terrain::ExtractShoreLine(int row, int col)
{
	// Extract single shoreline starting at (row,col).
	...
	ShoreLineType shore;						// return value
	PointDb ptAnchor(row, col);
	SetFlags(row, col, GetFlags(row, col) | static_cast<UInt8>(Flags::SHORE));

	// find/add adjacent shore points
	PointDb ptCur(row, col);
	while (true)
	{
		// examine neighbors
		bool found = false;
		for (int i = 0; i < 8; ++i)
		{
			// neighbor to be tested
			PointDb ptTest(ptCur.row + adj[i].row, ptCur.col + adj[i].col);
			if (ShoreEval(ptTest.row, ptTest.col))
			{
				if ((GetFlags(ptTest.row, ptTest.col) & static_cast<UInt8>(Flags::SHORE)) == 0)
				{
					// it evaluates to a shoreline point and is not already in a shoreline
					ptCur = ptTest;
					shore.pts.push_back(ptCur);
					SetFlags(ptCur.row, ptCur.col, GetFlags(ptCur.row, ptCur.col) | static_cast<UInt8>(Flags::SHORE));
					found = true;
					break;
				}
			}
		...
		if (found == false)
		{
			// closed shoreline, which should always happen
			if ((abs(ptCur.row - ptAnchor.row) < 4) && (abs(ptCur.col - ptAnchor.col) < 4))
			{
				Logger::Write(__FUNCTION__, "Closed shoreline.");
				break;
			}

			// if not back at beginning, we're stuck
			// backtrace until we find a valid neighbor
			for (int i = static_cast<int>(shore.pts.size()) - 2; i >= 0; --i)
			{
				if (found)
					break;

				// Note: this is essentially the same code as above, it might
				// be practical to merge it.
				ptCur = shore.pts[i];
	...
	// update max depth for this water body - there's a lot of inefficiency,
	// nested loops, etc. here
	...
	if (shore.pts.size() > ShoreLineType::MIN_PTS)
	{
		for (int row = 0; row < GetRowCount(); ++row)
		{
			for (int col = 0; col < GetColCount(); ++col)
			{
				if (IsWater(row, col))
				{
					// if it's within this shoreline (PointInPoly() doesn't work because
					// it apparently is only impelemented for convex polys

					// if we are in this water body, scanning in any direction
					// will hit one of its shoreline points (as long as init point is water)

					bool contains = false;
					for (int scanCol = col; (scanCol < GetColCount()) && (IsWater(row, col)); ++scanCol)
					{
						PointDb pt(row, scanCol);
						if (shore.ptSet.find(pt) != shore.ptSet.end())
						{
							contains = true;
							break;
						}
					...
					if (contains)
					{
						if (shore.maxDepth < GetDepth(row, col))
							shore.maxDepth = GetDepth(row, col);
					}
				}
			}
		}
...
int Terrain::GetShoreLine(int row, int col)
{
	// Return index of shoreline/waterbody containing point.
	//
	// Inputs:
	//		(row,col) = water point to test
	// Outputs:
	//		return = index of shoreline or -1 if not found
	//

	if (IsWater(row, col) == false)
		return -1;

//TODO:
//well, this idea didn't work out - apparently not 100% accurate
//& even if it were, unacceptably slow
//
// i think now the way to determine "within shoreline", is to have a 
// 2d map of all points in water body - it could be quickly created
// using the existing water floodfill algorithm
//
// then, "in shoreline" reduces to "map(row,col) == shoreindex"
//
	for (int i = 0; i < static_cast<int>(mv_ShoreLines.size()); ++i)
	{
		// if we are in this water body, scanning in any direction
		// will hit one of its shoreline points (as long as init point is water)

		for (int scanCol = col; scanCol < GetColCount(); ++scanCol)
		{
			PointDb pt(row, scanCol);
			if (mv_ShoreLines[i].ptSet.find(pt) != mv_ShoreLines[i].ptSet.end())
				return i;
		}
	}
...
int Terrain::GetShoreLineCount() const
{
	return static_cast<int>(mv_ShoreLines.size());
}

int Terrain::GetShoreLinePointCount(int shoreIndex) const
{
	int pointCount = 0;

	if ((shoreIndex >= 0) && (shoreIndex < GetShoreLineCount()))
		pointCount = static_cast<int>(mv_ShoreLines[shoreIndex].pts.size());

	return pointCount;
}

std::vector<Terrain::PointDb> Terrain::GetShoreLinePoints(int shoreIndex) const
{
	std::vector<PointDb> pts;

	if ((shoreIndex >= 0) && (shoreIndex < GetShoreLineCount()))
		pts = mv_ShoreLines[shoreIndex].pts;

	return pts;
}

double Terrain::GetMaxDepth(int shoreIndex) const
{
	// maximum water depth for specified water body, meters

	return mv_ShoreLines[shoreIndex].maxDepth;
}
