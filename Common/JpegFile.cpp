...
void j_putRGBScanline( UInt8* jpegline, int widthPix, UInt8* outBuf, int row );

void j_putGrayScanlineToRGB( UInt8* jpegline, int widthPix, UInt8* outBuf, int row );
...
UInt8* JpegFile::JpegFileToRGB( std::string fileName, unsigned int* width, unsigned int* height )
{
...
//		AfxMessageBox(buf);
	...
	*/
	...
	dataBuf = (UInt8*)new UInt8[cinfo.output_width * 3 * cinfo.output_height];
	if (dataBuf == NULL)
	{
...
bool JpegFile::GetJPGDimensions( std::string fileName, unsigned int* width, unsigned int* height )
{
	// basic code from IJG Jpeg Code v6 example.c
...
UInt8* JpegFile::RGBFromUInt32Aligned( UInt8* inBuf, unsigned int widthPix, unsigned int widthBytes, unsigned int height )
{
	if (inBuf == NULL)
		return NULL;

	UInt8* tmp;
	tmp = (UInt8*)new UInt8[height * widthPix * 3];
	if (tmp == NULL)
		return NULL;
							  ...
							  int quality )
{
	...
	UInt8* tmp;
	if (!color) {
		tmp = (UInt8*)new UInt8[widthPix*height];
		if (tmp==NULL) {
...
//------------------------------------------------------------------------------------------------------------------------
UInt8* JpegFile::MakeUInt32AlignedBuf( UInt8* dataBuf,
									 unsigned int widthPix,				// pixels!!
									 unsigned int height,
									 unsigned int* uiOutWidthBytes )	// bytes!!!
/*
	copies UInt8 buffer into UInt32-aligned UInt8 buffer - this means the entire buffer ends on a 4-byte boundary,
	not the individual pixels
...
*/
{
	...
	////////////////////////////////////////////////////////////
	// how big is the smallest UInt32-aligned buffer that we can use?
	unsigned int uiWidthBytes;
	uiWidthBytes = WIDTHBYTES(widthPix * 24);

	UInt32 dwNewsize = (UInt32)((UInt32)uiWidthBytes * (UInt32)height);
	UInt8* pNew;

	////////////////////////////////////////////////////////////
	// alloc and open our new buffer
	pNew = (UInt8*)new UInt8[dwNewsize];
	if (pNew == NULL)
		return NULL;
...
bool JpegFile::VertFlipBuf( UInt8* inbuf, unsigned int widthBytes, unsigned int height )
{
	UInt8  *tb1;
	UInt8  *tb2;
	...
	tb1 = (UInt8*)new UInt8[bufsize];
	if (tb1 == NULL)
	{
		return false;
	}

	tb2 = (UInt8*)new UInt8 [bufsize];
	if (tb2 == NULL)
	{
		delete [] tb1;
		return false;
...
bool JpegFile::BGRFromRGB( UInt8* buf, unsigned int widthPix, unsigned int height )
{
	if (buf == NULL)
		return false;
...
bool JpegFile::MakeGrayScale( UInt8* buf, unsigned int widthPix, unsigned int height )
{
	if (buf == NULL)
		return false;
